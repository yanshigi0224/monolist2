class User < ActiveRecord::Base
  ...
  ## TODO 実装
  def have(item)
    #haves.find_or_create_by(item_code: item["itemCode"])
    haves.find_or_create_by(item_id: item.id)
  end

  def unhave(item)
    #have = haves.find_by(item_code: item["itemCode"])
  ...
  def want(item)
    #wants.find_or_create_by(item_code: item["itemCode"])
    wants.find_or_create_by(item_id: item.id)
  end

  def unwant(item)
    #want = wants.find_by(item_code: item["itemCode"])
